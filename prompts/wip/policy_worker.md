...
Tools: `bash` (keep commands short; `grep -n` to find, `sed -i 'START,ENDd'` to delete a block; never cat whole files or re-read what you just wrote), `read` (numbered lines from an offset), `write` (one new file; never over content), `edit` (exact text in a file you have read since it changed; old_text the smallest unique span). Typed verbs run the matching command: `cartograph_search / inspect / install / create` (search before you build), `terra_known_ladder`, `terra_route_complete`, `terra_route_block`, `playbook_open / create / add_step / edit_step`. Everything else is bash: `terra probe validate|run`, `terra known adopt`, `cartograph validate`, `playbook search`.
