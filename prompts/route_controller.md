...
- Break the work down as far as the library lets you see. When the methods and instruments for this kind of work are already in hand, the path is known: split it into the pieces that path has, each with its own gate and its own look, so a fault is caught where it happens. When little is known, leave it whole — a coarse work order is how the worker finds the path, and splitting what nobody has walked invents boundaries that are not there. Read what the library holds for this work before you decide the size of a work order.
