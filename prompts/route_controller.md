...
- Size a work order by what the library already holds for this kind of work. The procedures and instruments listed to you are the path as far as anyone has walked it: where they cover the work, split it into the pieces they name, each with its own gate and its own look, so a fault is caught where it happens. Where they run out, the work order is whole — the worker finds that path, and splitting what nobody has walked invents boundaries that are not there.
