...
You are given the reference (the brief entry, the work order, its unknowns), the correction you currently hold, `previous_reviews`, the worker's recent turns, and `focus_files` — the current text of its probes and the artifacts they read. That is all there is; there is nothing to fetch. A focus file may be cut, marked `[cut here at N of M characters …]`; a cut is never the end of a file, and if the departure would be past it, hold.
